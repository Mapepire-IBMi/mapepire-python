...
- fix: pin `ruff>=0.15,<0.16` — it was previously unconstrained, and `ruff` 0.16.0 (released after `uv.lock` was last generated) expanded its default enabled rules, breaking the Lint job with 395 new findings on a fresh, uncached `pip install -e .[dev]`
- added unit testing for kerberos
